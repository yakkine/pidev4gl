...
import tn.esprit.pidev.interfaces.CommentServiceLocal;
...
public class CommentService implements CommentServiceLocal {
   ...
   em.remove(commentaire);
	...
	public List<Comment> showCommentairebyDocument(int idDocument) {
		...
		TypedQuery<Comment> query = em.createQuery("SELECT c FROM comment c ,document d WHERE c.documment_id=:d.id", Comment.class);
	...
	public List <Comment> getAllEmploye() {
		...
		TypedQuery <Comment> query= em.createQuery("select e from Comment e where e.documment=1 ",Comment.class);
